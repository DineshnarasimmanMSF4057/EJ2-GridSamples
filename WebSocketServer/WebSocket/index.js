...
let data1=data;
const newData=data1.slice(0,15);
const totalCount=data.length;
const dataTosend=JSON.stringify([{resultData:newData,resultCount:totalCount}])
...
sockserver.on('connection', ws => {
 ...
 ws.on('message', function incoming(message) {
    ...
    if(request?.action?.requestType==='infiniteScroll')
    { // handling the data for infiniteScroll and sending data to the client
        console.log('infiniteScroll');
        const scrollData=data.slice(request.action.startIndex,request.action.startIndex+15);
        
        const infiniteScrollData=JSON.stringify([{resultData:scrollData,resultCount:totalCount}])
        ws.send(infiniteScrollData)
